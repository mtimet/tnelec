...
ELEC.main = function(){
    ...
    function updateVoteBar(){
        var bar = d3.select('#vote-bar')
            .style('display',(ELEC.state.currentCirconscription && ELEC.state.currentParty == null) ? 'block' : null);
        var circonscription = ELEC.state.currentCirconscription;
        if (circonscription){
            var agg = getCirconscriptionAgg(circonscription),
                registred = agg.electeurs.enregistre,
                voted = agg.electeurs.votant,
                abstention = Math.floor(10000 * (registred - voted)/registred) / 100 + '%',
                valid =  agg.bulletins.correct + '|'+ Math.floor(10000 * (agg.bulletins.correct / voted)) / 100 + '%', 
                blank = Math.floor(10000 * (agg.bulletins.blancs / voted)) / 100 + '%', 
                invalid = Math.floor(10000 * (agg.bulletins.annule / voted)) / 100 + '%';

            bar.select('#registred')
                .text(registred);
            bar.select('#voted')
                .text(voted);
            bar.select('#abstention')
                .text(agg.electeurs.enregistre);
