...
ELEC.main = function(){
    ...
    var representativesPerCirconscription,
        resultsPerParty,
    ...
    function updatePartyBar(){
        ...
        if (ELEC.state.currentParty) {
            var i = data.elus.map(function(d){return d.id;}).indexOf(ELEC.state.currentParty);
            var barData  = [(ELEC.state.currentCirconscription ? representativesPerCirconscription[ELEC.state.currentCirconscription][ELEC.state.currentParty] : data.elus[i])];
    ...
    function updateInfoBox() {   
        ...
        if (ELEC.state.currentParty){
            if (ELEC.state.currentCirconscription){
                infoData = d3.entries( CIRCONSCRIPTIONS[ELEC.state.currentCirconscription].delegations)
                    .map(function(_){
                        var r = resultsPerCirconscriptionPerParty[ELEC.state.currentCirconscription][ELEC.state.currentParty];   
                        return {name:_.value.name,pourcentage:r[_.key].pourcentage, vote:r[_.key].vote, id : 'c'+ ELEC.state.currentCirconscription +'d' +_.key};
                    })
            } else {
                infoData = d3.entries(resultsPerParty[ELEC.state.currentParty]);
            ...
            tr.enter()
                ...
                .style('cursor', ELEC.state.currentCirconscription ? null : 'pointer');
            tr.append("td")
                .text(function(_){
                    return ELEC.state.currentCirconscription ? _.name : CIRCONSCRIPTIONS[_.key].name;
                });
            tr.append('td')
                .text(function(_){return Math.floor( (ELEC.state.currentCirconscription ?_.pourcentage : _.value.pourcentage) * 100) / 100 +"%"; } )
                .classed("percentageBig",true);

            tr.append("td")
                .text(function(_){
                    return formatNumber(ELEC.state.currentCirconscription ?_.vote : _.value.vote);
                });
            if (!ELEC.state.currentCirconscription){
                tr.append("td")
    	    ...
    	    if (ELEC.state.currentCirconscription){
                infoData = d3.entries(representativesPerCirconscription[ELEC.state.currentCirconscription]).map(function(_){var r = {}; r = _.value; r.id = _.key;r.pourcentage = r.percent; return r}).sort(function(a,b){return a.vote < b.vote});
            } else {
                infoData = data.elus;
            }
    ...
    function mouseover2(d,i){
        ...
        .text( ELEC.state.currentCirconscription &&  d.properties.name_2? d.properties.name_2 : d.properties.name_1);
        d3.select(this)
        .style('cursor', ELEC.state.currentCirconscription &&  d.properties.name_2 ? 'auto' : null);
        d3.select("#tooltipInstruction")
        .style('display',  ELEC.state.currentCirconscription &&  d.properties.name_2 ? 'none' : null);

        tbl.select('tbody').remove();
        var tooltipData = d.properties.agg.resultat.listes.filter(function(d,i){return i < 5;});
        if (ELEC.state.currentParty || ELEC.state.currentCirconscription){
            tooltipData = ( delegationModeActive(d) && d.properties.code_circo == ELEC.state.currentCirconscription) ? d.properties.agg.resultat.listes.filter(function(d,i){return i < 5;}) : [];
        } else {
        ...
        if (ELEC.state.currentParty){ 
            ...
            if (!ELEC.state.currentCirconscription){
                if(representativesPerCirconscription[d.properties.code_circo][ELEC.state.currentParty]) partyResult.elus = representativesPerCirconscription[d.properties.code_circo][ELEC.state.currentParty].elus;
            }
    ...
    function selectCirconscription(id){
        ...
        ELEC.state.currentCirconscription = id;
        ...
        if (ELEC.state.currentCirconscription) {	
            d3.json('data/geojson/'+CIRCONSCRIPTIONS[ELEC.state.currentCirconscription].shp+'.json',function(circoGeojson){
    	    //FIXME: get the election data per circo here
        	    d3.json('data/flat/circonscriptions/' + ELEC.state.currentCirconscription +'.json',function(flatResults){
                    resultsPerCirconscriptionPerParty[ELEC.state.currentCirconscription] = flatResults;
        		    circoGeojson.features.forEach(function(d){d.properties.code_circo = ELEC.state.currentCirconscription}); //FIXME: on the geojson
