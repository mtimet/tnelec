...
function mouseover2(d,i){
	...
	.text( (selectedCirconscription && d.properties.code_deleg ) ? d.properties.name_2 : d.properties.name_1);
    
    tbl.select('tbody').remove();
    var tooltipData = selectedCirconscription && d.properties.agg ? d.properties.agg.resultat.listes.filter(function(d,i){return i < 5;}) : d.properties.elus.listes;
    if (selectedCirconscription){
	tooltipData = (d.properties.code_deleg && d.properties.agg) ? d.properties.agg.resultat.listes.filter(function(d,i){return i < 5;}) : [];
    } else {
...
function id(d){
    return "c" + d.properties.code_circo + (d.properties.code_deleg ? "d" + d.properties.code_deleg :"");
}
...
function fetchDelegationData(circonscriptionId, delegationId, callback) {
	...
	delegationData ? callback(null, { agg : delegationData, id : delegationId}) : callback("error", null);
    });
}
...
function fetchAllDelegationsPerCirconscription(geojson, callback){
    ...
    q.await(function(error, results) { 
	var delegations = results.map(function(_){return _.id});
	geojson.features
	    .forEach(function(feat){
		var id = feat.properties.code_deleg;
		var i = delegations.indexOf(id);
		if(i > -1){
		    feat.properties.agg = results[i].agg;
		}
	    });
...
function selectCirconscription(id){
    ...
    if (previousCirconscription) data.features = data.features.filter(function(d){return (typeof d.properties.code_deleg === "undefined")});
...
var ELEC = (function() {
	...
	if (selectedParty){
		...
		var first = d.properties.elus ? d.properties.elus.listes[0] : null;
		if (selectedCirconscription){
		    if( typeof d.properties.code_deleg === "undefined") return;
		    if (d.properties.agg) first = d.properties.agg.resultat.listes[0];
		}
