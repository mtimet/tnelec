var PARTIES = {"464" : { "name-en": "Ennahdha", fill: "#183883",logo:"ennahdha.jpg"},
	...
	"330" : {"name-en":"UDC",      "longname-fr":"Union des Diplomes Chommeurs", fill:"#2b2f86",logo:"udc.jpg"}, //la liste est rattache a l'UDC d'ou le shortcut pour le nom
...
function mouseover2(d,i){
    ...
    d3.select(this)
	.style('cursor', selectedCirconscription &&  d.properties.name_2 ? 'auto' : null);
    d3.select("#tooltipInstruction")
	.style('display',  selectedCirconscription &&  d.properties.name_2 ? 'none' : null);
   
    tbl.select('tbody').remove();
    var tooltipData = d.properties.agg.resultat.listes.filter(function(d,i){return i < 5;});
    if (selectedParty || selectedCirconscription){
	...
	if (!selectedCirconscription){
	    if(representativesPerCirconscription[d.properties.code_circo][selectedParty]) partyResult.elus = representativesPerCirconscription[d.properties.code_circo][selectedParty].elus;
	}
...
function selectCirconscription(id){
    ...
    navigationButton.style('display', function(){return selectedParty || selectedCirconscription ? 'inline-block' : null})
